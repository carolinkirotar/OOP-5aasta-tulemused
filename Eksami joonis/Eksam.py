...
ax = sns.violinplot(data=df_copy, x="aasta", y=focused_row, inner="quart", linewidth=1, scale="count", color='#d06464')
...
# sns.stripplot(data=df_copy, x="aasta", y=focused_row, size=8, color="none", edgecolor="grey", linewidth=1, dodge=True, alpha=0.5)
...
for i in range(len(means)):
    ...
    box_plot.annotate(text, xy=(i, means[i]), xytext=(i+0.04, means[i]-3.7), ha='left', fontsize='8',
...
for tick, label in zip(pos, box_plot.get_xticklabels()):
   box_plot.text(pos[tick], -2.3, nobs[tick],
            ha='center',
            size='small')


plt.ylim(-0.3, 35)
plt.ylabel('Eksamitöö', fontweight='bold')
