...
ax = sns.violinplot(data=df_copy, x="aasta", y=focused_row, inner="quart", linewidth=1, scale="count", color='#d3436e')
...
for i in range(len(means)):
    ...
    box_plot.annotate(text, xy=(i, means[i]), xytext=(i+0.04, means[i]-1.9), ha='left', fontsize='8',
...
for tick, label in zip(pos, box_plot.get_xticklabels()):
   box_plot.text(pos[tick], -1.4, nobs[tick],
            ha='center',
            size='small')


plt.ylim(-0.3, 17)
plt.ylabel('Kontrolltöö 1', fontweight='bold')
