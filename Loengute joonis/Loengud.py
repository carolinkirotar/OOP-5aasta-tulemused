...
ax = sns.violinplot(data=df_copy, x="aasta", y=focused_row, inner="quart", linewidth=1, scale="count", color='#e4e5eb')
...
for tick, label in zip(pos, box_plot.get_xticklabels()):
   box_plot.text(pos[tick], -1.0, nobs[tick],
            ha='center',
            size='small')


plt.ylim(-0.3, 13)
plt.ylabel('Loengupunktid', fontweight='bold')
